...
## 本次新增：愤怒的小鸟（2026-09-17）

- Java 21 后端 **103 项测试通过**。新增引擎、平台和快照测试，覆盖 2–10 人、1–6 只炸弹的严格设置校验、随机顺序、位置不重复、循环行动、安全飞走、爆炸揭晓、10 秒超时自动点击、离线代点及手动点击与维护任务并发时只结算一次。
- 个人 / 公共视图在结束前不泄露炸弹位置；校验越权、重复位置、旧回合、旧局次、重复 requestId、下一局重置、JSON 快照恢复与旧设置默认值。无积分、挑战事件。
- 新增 **3 项 Playwright 测试通过**：真实三人房间覆盖设置失败回退与延迟确认、多人同步、键盘点击、REST / WebSocket / ACK 保密、结果一致、刷新、离线超时自动点击及下一局；两项受控快照测试分别验证普通动画和减少动态效果模式下，飞走 / 爆炸只播放一次，重复 ACK 和刷新不重播。
- 原有 **7 项浏览器回归测试通过**，涵盖其他七款游戏、大厅同步、房主转移和首页四种宽度。人工检查 375px 手机、1440px 桌面棋盘与爆炸结果截图，375 / 390 / 430px 棋盘保持四列且无页面横向溢出。
- 前端生产构建、TypeScript 检查（含新增浏览器测试）、Prettier 检查、`git diff --check` 和真实 REST / WebSocket 协议冒烟检查通过。

本次使用本地临时内存模式后端与 Vite 联调；Redis 兼容性通过共用快照编解码及游戏状态 JSON 恢复测试验证，未额外执行真实 Redis 重启测试或实体手机测试。未更新已有部署。

稳定截图：[手机棋盘](screenshots/angry-birds-mobile.png)、[桌面棋盘](screenshots/angry-birds-desktop.png)、[爆炸结果](screenshots/angry-birds-result.png)。首页截图已更新为八款游戏。
